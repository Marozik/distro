(() => {
    ...
    da.init();
    const second = 1e3, minute = 60 * second, hour = 60 * minute, day = 24 * hour, week = 7 * day;
    let countDown = new Date("june 27, 2023 00:00:00").getTime();
    setInterval((function() {
        let now = (new Date).getTime(), distance = countDown - now;
        document.getElementById("weeks").innerText = Math.floor(distance / week), document.getElementById("days").innerText = Math.floor(distance / day), 
        document.getElementById("hours").innerText = Math.floor(distance % day / hour), 
        document.getElementById("minutes").innerText = Math.floor(distance % hour / minute);
    }));
    const input1 = document.querySelector(".range-sale__input");
    const progressBar = document.querySelector(".range-sale__progress-bar");
    if (input1.value > 0) showProgress();
    function showProgress() {
        let min = +input1.min, max = +input1.max - min, value = +input1.value - min, c = 0;
        const result = value / max * 100;
        if (result < 0) c = 0; else c = `${result}`;
